<?php defined('ACCESS_SUBFILES') or die('Restricted access');
			...
			require_once("classes/giftcoupons.php");

			$action = $_GET['action'];
			
			$id=(int)$_GET['id'];

			$page_url="index.php?p=giftcoupons";

			$page_url2="index.php?p=giftcoupons";	
...
<div class="box">
	...
	<table border="0" cellpadding="0" cellspacing="0" style="width:720px;">
		...
		<?php			
			...
			$catg_select_query="SELECT * FROM ".TABLE_GIFTCOUPONS." ORDER BY gift_id";
		...
		<tr>
			...
			<th class="col2">Gift Copon Name</th>
			<th class="col2">Gift Coupon Code</th>
			<th class="col2">Gift Value</th>
...
<?php	
		...
		<tr >
			...
			<td><?php echo stripslashes($catg_select_rows['gift_name']);?></td>
			<td><?php echo stripslashes($catg_select_rows['gift_coupon']);?></td>
			<td><?php echo $catg_select_rows['gift_type'];?><?php echo stripslashes($catg_select_rows['gift_value']);?></td>
		  	<td class="action">

					<li>

					<a class="edit" href="<?php echo $page_url;?>&action=edit&id=<?php echo $catg_select_rows['gift_id'];?>" title="edit"></a>

					</li>

					<li><a class="delete" href="<?php echo $page_url;?>&action=delete&id=<?php echo $catg_select_rows['gift_id'];?>" onclick="if(!confirm('Are you sure to delete this?')) return false;" title="delete"></a></li>
