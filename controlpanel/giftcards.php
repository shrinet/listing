<?php defined('ACCESS_SUBFILES') or die('Restricted access');
			...
			require_once("classes/giftcards.php");

			$action = $_GET['action'];
			
			$id=(int)$_GET['id'];

			$page_url="index.php?p=giftcards";

			$page_url2="index.php?p=giftcards";	
...
<fieldset>
	...
	<label>Gift Card Send By :</label>

	<span class="small_input"><input class="small" name="send_email" type="text" value="<?php echo htmlentities($data['send_email']);?>" /></span><br class="hid" />
	<label>Gift Card Recieved By :</label>

	<span class="small_input"><input class="small" name="reciepient_email" type="text" value="<?php echo $data['reciepient_email'];?>" /></span><br class="hid" />

	<label>Gift Card Code:</label>

	<span class="small_input"><input class="small" name="token" type="text" value="<?php echo $data['token'];?>" /></span><br class="hid" />
	
	<label>Gift Card Amount:</label>

	<span class="small_input"><input class="small" name="membership_type" type="text" value="<?php echo $data['membership_type'];?>" /></span><br class="hid" />
...
<div class="box">
	...
	<table border="0" cellpadding="0" cellspacing="0" style="width:720px;">
		...
		<?php			
			...
			$catg_select_query="SELECT * FROM ".TABLE_GIFT_COUPONS." ORDER BY id";
		...
		<tr>
			...
			<th class="col2">Gift Card Send By</th>
			<th class="col2">Gift Card Recieved By</th>
			<th class="col2">Gift Card Code</th>
			<th class="col2">Gift Card Amount</th>
...
<?php	
		...
		<tr >
			...
			<td><?php echo stripslashes($catg_select_rows['send_email']);?></td>
			<td><?php echo stripslashes($catg_select_rows['reciepient_email']);?></td>
			<td><?php echo $catg_select_rows['token'];?></td>
			<td><?php echo '$'.$catg_select_rows['membership_type'];?></td>
		  	<td class="action">
